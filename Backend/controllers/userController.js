  ...
  try {
    ...
    if (!name || !username || !email || !password || !role) {
      return res.status(400).json(
          {message: 'Somethings are missing', success: false})
    }
    ...
    return res.status(201).json(
        {message: 'Account creatd successfully', success: true})
  } catch (error) {
    console.log(error)
  }
}
...
export const login = async (req, res) => {
  try {
    ...
    const token =
        await jwt.sign(tokenData, process.env.SECRET_KEY, {expiresIn: '1d'});
